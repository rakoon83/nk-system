...
async function deleteSelectedRows() {
  ...
  openConfirm({
    mountId: "modal-root",
    title: "삭제 확인",
    message: `선택한 ${num(ids.length)}건의 출고 검수 로그를 삭제할까요?`,
    onConfirm: async () => {
...
async function deleteOldRows() {
  openConfirm({
    mountId: "modal-root",
    title: "60일 이전 로그 삭제",
    message: "60일 이전 출고 검수 로그를 삭제할까요?",
    onConfirm: async () => {
...
function downloadExcel() {
  ...
  downloadExcelFile({
    fileName: "scan_out_log.xlsx",
    sheetName: "scan_out_log",
    rows
  });
