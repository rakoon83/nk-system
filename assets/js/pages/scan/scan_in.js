...
async function loadInvoice() {
  ...
  try {
    const [docRows, itemRows, wmsData, defectRows, specialData, barcodeData] = await Promise.all([
    ...
    playSound("modal");
    setPageStatus(`${num(scanRows.length)}건 조회 / 바코드 ${num(barcodeRows.length)}건`);
