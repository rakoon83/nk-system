...
async function init() {
  ...
  tableManager.setStatus("데이터 준비 중...");
...
async function syncInvoiceProgress() {
  ...
  if (!sapDocRows.length) {
    tableManager.setStatus("sap_doc 데이터 없음");
    return;
  }

  const oldMap = new Map();
  oldRows.forEach(row => oldMap.set(clean(row.invoice_display), row));
  ...
  if (upsertRows.length) {
    ...
    if (error) {
      console.error(error);
      tableManager.setStatus("invoice_progress 동기화 실패");
      return;
    }
  }
  ...
  tableManager.setStatus(`자동동기화 완료 ${num(upsertRows.length)}건`);
}
...
async function loadRows() {
  ...
  allRows = rows.map(row => {
    ...
    return {
      ...
      load_status: getLoadProgress(invoice)
    };
  });
...
function getLoadProgress(invoice) {
  const exists = loadRows.some(row => clean(row.invoice) === invoice);

  if (exists) {
    return { text: "검수완료", done: 1, total: 1, className: "progress-done" };
  }

  return { text: "미검수", done: 0, total: 1, className: "progress-none" };
}
